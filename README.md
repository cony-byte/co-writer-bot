...
bot/tool_router_slack.py # Slack 확인 버튼·pending·검증 후 실행
...
복합 요청은 순서 있는 함수 계획으로 반환한다. 코드는 모든 인자와 첨부 ID를 먼저 검증하며,
자연어에서 나온 실행은 위험도와 관계없이 정확한 Slack 실행 버튼 확인을 강제한다.
