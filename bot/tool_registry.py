...
_writing = {
    ...
    "sync_notion": "★★★노션 URL이 메시지에 있다는 사실만으로는 절대 호출하지 마라 — 이 툴을 호출할 유일한 조건은 사용자가 '동기화'/'싱크'/'연동'이라는 단어를 실제로 썼을 때뿐이다(그 외 조건 없음). 예를 들어 '<URL> 날혐남 1화 개요 생성'처럼 URL과 다른 작업 요청이 같이 왔는데 '동기화'라는 말이 없으면, 그 URL은 '어떤 작품 얘기인지' 알려주는 참조일 뿐이니 sync_notion을 호출하지 말고 요청한 작업(예: generate_script)만 바로 호출해라 — 재동기화는 몇 분씩 걸려서, 잘못 호출하면 사용자가 실제로 원한 작업이 몇 분간 안 되고 멈춘다. '노션에 있는 자료 참고해서 만들어줘'류 요청에도 쓰지 않는다.",
