...
INTENT_SPECS: dict[str, dict] = {
    ...
    "help": {"label": "도움말", "desc": "사용법, 명령 목록, 무엇을 할 수 있는지 전체 안내를 요청."},
    "confirm_previous": {
        "label": "직전 제안 승인",
...
R7. 취소는 명시적일 때만 cancel_job이다. "그만", "중단", "멈춰"도 반드시 cancel_job으로 분류한다. "도움말", "help", "사용법"은 help다. 자연어 카드 답변과 재개 요청도 regex가 아니라 현재 상태와 최근 봇 질문을 보고 confirm_previous/reject_previous/resume_interrupted로 분류한다.
...
★출력하는 work는 항상 registered_works의 정식명(딕셔너리 키)이어야 한다 — 매치된 표기가 별칭(값 배열 안의 문자열)이면 그 별칭 문자열 그대로 쓰지 말고 그 별칭이 속한 정식명으로 바꿔서 채운다. 예: registered_works={"저는 연프 출연진이 아닌데요 !": ["저연프"]}이고 문장에 <저연프> 또는 [저연프] 또는 thread_parent_text="저연프"로만 나와도, work='저는 연프 출연진이 아닌데요 !'다(work='저연프' 아님).
...
FEW_SHOTS: list[tuple[str, dict]] = [
    ...
    ("도움말", {"intent": "help", "confidence": 0.99}),
    ...
    (
        "하루, 겨울 인물 등록하고 나서 씬3 스틸컷도 바로 뽑아줘",
        {
            "intent": "element_register",
            "steps": [
                {
                    "intent": "element_register",
                    "elements": [
                        {"kind": "인물", "name": "하루", "image_index": 0},
                        {"kind": "인물", "name": "겨울", "image_index": 1},
                    ],
                },
                {"intent": "stillcut", "scene": 3},
            ],
    ...
    (
        "[저연프] 인물 뭐 등록됐어?",
        # registered_works={"저는 연프 출연진이 아닌데요 !": ["저연프"]}인 상황 — "저연프"는 별칭이므로
        # work는 그 별칭이 속한 정식명(딕셔너리 키)으로 채운다, 매치된 별칭 문자열 그대로 두지 않는다.
        {
            "intent": "answer_question",
            "work": "저는 연프 출연진이 아닌데요 !",
...
★steps를 채울 때는 그 정보를 최상위 필드(예: elements)에 중복해서 또 넣지 마라 — 최상위는 steps[0]과
같은 값이면 생략(null)해도 된다. 중복은 출력 길이를 불필요하게 늘려 응답이 중간에 잘리는 사고로
이어진다.
