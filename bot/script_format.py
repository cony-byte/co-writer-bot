...
# ---------------- 변환 프롬프트 ----------------
CONVERT_SYSTEM = ("넌 방송대본 포맷 정리 전문가다. 역할은 오직 형식 재편이며 창작이 아니다. "
                  "초안에 실제로 있는 문장만 블록으로 옮기고, 단 한 줄도 새로 지어내지 않는다.")

CONVERT_USER_TMPL = """아래 [초안]을 촬영대본 구조로 재편해서 JSON으로만 출력하라.
너의 일은 '옮겨 담기'다. 초안에 있는 문장만 블록으로 바꾸고, 없는 건 만들지 않는다.
...
4. 애매하면 원문을 보존하라.
5. **초안의 마지막 문장이 곧 대본의 끝이다.** 그 뒤로 새 대사·지문·씬·엔딩·훅을 **절대 이어쓰지 마라.** 초안이 중간에서 끊겨도 거기서 멈춘다.
6. 출력 블록은 초안에 실재하는 줄에서만 나온다 — 초안에 없는 인물·장면·대사를 추가하면 안 된다.
