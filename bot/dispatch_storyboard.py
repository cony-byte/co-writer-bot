...
# ★2026-07-20 "동시 생성이어도 4컷씩 끊어서 뱉어라" — 예전엔 40컷 미만이면 통짜 그리드 하나로
# 올려서(실측 34컷이 거대한 한 장으로 나옴) 알아보기 힘들었다. 이제 한 그리드당 이 개수로 고정.
_GRID_BATCH = 4
...
def _render_cuts(channel, thread_ts, work, bible, source_text, *,
    ...
    # (2026-07-20) 한 그리드당 _GRID_BATCH(4)컷으로 고정해 순서대로 올린다 — 컷이 많아도
    # 통짜 한 장으로 뭉치지 않고 4컷씩 끊어서 뱉어(읽기 쉽게, 사용자 요청). 4컷 이하면 1장.
    batches = [panels[i:i + _GRID_BATCH] for i in range(0, len(panels), _GRID_BATCH)] or [panels]
