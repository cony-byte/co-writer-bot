...
# 스토리보드 이미지는 생성된 전체 컷을 한 장의 그리드로 합친다.
# 스틸컷 생성의 4컷 배치 로직과는 별개다.
...
def _render_cuts(channel, thread_ts, work, bible, source_text, *,
    ...
    # ★2026-07-22 2D 웹툰 화풍 여부 — 각 컷 프롬프트 끝에 참조 일관성 문구를 붙일지 결정.
    try:
        _is_2d_style = bool(work and works.get_style(work) == "2d_anim")
    except Exception:
        _is_2d_style = False
    ...
    def _gen_group(start, end):
        ...
        for i in range(start, end):
            ...
            if role_block:
                prompt = f"{prompt}\n\n{role_block}"
            # ★2026-07-22 2D 웹툰 화풍: 각 컷 끝에 참조 이미지와 화풍·선화·컬러링을 일치시키라는
            # 지시를 붙여 컷 간 스타일 일관성을 강화(사용자 지정).
            if _is_2d_style:
                prompt = (f"{prompt}\n\nKeep the art style, line work, and coloring consistent "
                          f"with the attached reference image.")
    ...
    # 스틸컷의 4컷 단위 생성·업로드 로직은 변경하지 않는다.
    miss = n - len(panels)
    _update_note(channel, ph, f"{len(panels)}컷 전체를 한 장의 그리드로 합치는 중이에요…")
    try:
        grid_png = grid.build_grid(
            panels, cols=cols or config.OPENROUTER_GRID_COLS, no_text=no_text
        )
    except Exception:
        log.exception("grid failed")
        _post_chunks(channel, thread_ts, "이미지 합성 중 오류가 났어요. 잠시 후 다시 시도해주세요.")
        return

    cap = f"🖼️ {title} — {len(panels)}컷"
    ...
    try:
        app.client.files_upload_v2(
            ...
            filename=fname, title=f"{title} {len(panels)}컷", initial_comment=cap,
        )
    except Exception:
...
def _post_element_candidate(channel, thread_ts, work, name, etype, context, force: bool = False,
    ...
    pending_element_state.set(
        resp["ts"], work=work, name=name, etype=etype, context=context, png=png,
...
STYLE_PRESETS = {
    ...
    # ★2026-07-22 사용자 지정 — 2D는 '일본 애니메'가 아니라 '2D 웹툰' 스타일(학원 로맨스 웹툰):
    # 클린 라인아트 + 플랫 컬러링 + 밝고 저채도(깊거나 과채도 톤 회피). 컷 간 팔레트·선화 일관성.
    "2d_anim": (
        "2D webtoon illustration style, clean line art, flat coloring, bright and low-saturation, "
        "avoiding deep or heavily saturated tones, bright, flat, naturally lit look appropriate for "
        "a school romance-drama webtoon, kept airy and low-saturation rather than deep or heavily "
        "warm. Consistent color palette and linework across all cuts. "
        "Not photorealistic, not 3D-rendered/CGI, not a photograph, no film-camera lens effects. "
        "A clearly fictional character, not resembling any real celebrity or public figure. "
        f"{_STYLE_COMMON_SUFFIX}"
    ),
}
...
_ELEMENT_REF_STYLE_PHRASE = {
    ...
    # ★2026-07-22: 2D는 웹툰 화풍(클린 라인아트+플랫 컬러링+밝은 저채도)으로 통일 — 요소
    # 참조샷도 스틸컷과 어긋나지 않게 같은 화풍 문구를 쓴다.
    "2d_anim": "2D webtoon illustration, clean line art, flat coloring, bright low-saturation",
}
...
def _generate_video_for_cut(channel, thread_ts, work, title, cut, num, scene_seconds,
    ...
    # ★2026-07-22 실무자 레퍼런스 반영 — 대사 없는 컷은 입이 제멋대로 말하는 모양으로 움직이는
    # 문제가 있어(무대사인데 발화 입모양) 입을 다물고 발화 모양을 만들지 말라고 명시한다.
    # 대사 있는 컷은 자연스러운 발화를 위해 이 잠금을 걸지 않는다.
    dialogue_lock = ("" if _cut_has_dialogue(cut) else
    ...
    motion_prompt = (f"{grid_anchor}{fiction_lock}{ref_lock}{camera_lock}{style_lock} "
                     f"{tags} {cut['prompt']}. Scene action: {cut['caption']}. {dialogue_lock}"
...
def _save_ref_pairs(work, etype, pairs):
    ...
    for nm, ext, data, _url in pairs:
        ...
        if etype == "person":
            try:
                from . import face_ref
                res = face_ref.make_face_reference(data)
                if res.png is not res.original:
                    arch = config.OPENROUTER_REFS_DIR / oi.canon_work(work) / "_originals"
                    arch.mkdir(parents=True, exist_ok=True)
                    (arch / f"{nm}{ext}").write_bytes(data)
                    data, ext = res.png, ".png"
                if res.note:
                    FACE_REF_NOTES.append(res.note)
            except Exception as e:
                log.warning("face_ref 실패(%s) — 원본 그대로 등록", e)
        if vpfx is not None:
...
def _parse_reference_element_target(query: str) -> tuple[str | None, int | None, str, str]:
    ...
    # "이 이미지에 있는 거랑"처럼 이미지/사진 언급 뒤에 "있는 거랑/것처럼/대로" 등이 붙는
    # 흔한 어순은 위 패턴들이 못 걷어내 "…있는 거랑 이미지" 같은 쓰레기 라벨로 남는다
    # (실사용 사고: display_label 미채움 시 이 함수 결과가 그대로 노출/등록됨).
    q = re.sub(r"있는\s*(?:거랑|거처럼|것처럼|대로)", " ", q)
    q = re.sub(r"(?:이미지|사진)", " ", q)
...
def _do_element_ref_generate(channel, thread_ts, query, event, work=None, name=None, etype=None,
                             extra_alias: str | None = None) -> bool:
    ...
    재파싱 없이 그 값을 그대로 쓴다. name은 호출부(nl_router.execute)가 이미 display_label로
    치환해 넘기므로 여기서는 그대로 쓴다 — extra_alias는 사용자가 원래 부른 표현으로,
    등록 시 별칭으로 함께 저장해 다음부터 어느 쪽으로 불러도 매칭되게 한다."""
    q = query or ""
...
def _do_typed_element_regen(channel, thread_ts, cmd, rest) -> None:
    ...
    브래킷 명령 [장소]/[의상]/[소품]/[인물]로 등록된 요소의 참조 이미지를 AI로 다시 생성한다.
    예전엔 이 명령들이 어느 커맨드 셋에도 없어서 co-writer LLM으로 새(→"이미지 생성 못 함" 거절).
    cmd: '장소'/'의상'/'소품'/'인물' 등(_REF_TYPE_KW 키). rest: '<작품> 이름 이미지 다시 만들어줘'."""
    etype = _REF_TYPE_KW.get(cmd.strip().lower(), "person")
    q = (rest or "").strip()
    work = None
    wm = SUB_RE.match(q)
    if wm and not _looks_like_mention(wm.group(1)):
        work = wm.group(1).strip()
        q = wm.group(2).strip()
    # 이름만 남기려 꼬리의 "이미지/다시/만들어줘/그려줘/재생성/새로" 등 지시어를 걷어낸다.
    name = re.sub(r"(이미지|사진|을|를|다시|재생성|새로|또|한\s*번\s*더|만들어\s*줘?|만들어|"
                  r"생성해\s*줘?|생성|그려\s*줘?|그려|해\s*줘?|해)", " ", q)
    name = re.sub(r"\s+", " ", name).strip(" ,·-").strip()
    if not name:
        _reply(channel, thread_ts,
               f"어떤 {cmd}을(를) 다시 만들지 이름을 알려주세요 — 예: `[{cmd}] <작품> 대기실 이미지 다시 만들어줘`.")
        return
    if not work:
        joined = "\n".join(mm["content"] for mm in _thread_messages(channel, thread_ts))
        work = _work_from_thread(joined, thread_ts)
    if not work:
        ...
        return
    work = works.resolve(work) or work
    _reply(channel, thread_ts, f"🎨 <{work}> {name} {_ELEMENT_TYPE_LABEL.get(etype, etype)} 이미지를 AI로 다시 생성할게요…")
    _post_element_candidate(channel, thread_ts, work, name, etype, context="")
...
_CAPCUT_EDIT_SYS = (
    ...
    '- {"op":"mute","audio_no":N}\n'
    '- {"op":"add_text","cut_no":N,"text":"자막 내용","size":글자크기(선택,기본8),'
    '"color":"#RRGGBB"(선택,기본흰색),"x":가로위치(선택,-1~1,기본0),"y":세로위치(선택,-1~1,'
    '기본0, 화면 하단 근처는 -0.8 정도)}  (그 컷 구간에 맞춰 자막 추가)\n'
    '- {"op":"edit_text","text_no":N,"text":"새 자막 내용"}\n'
    '- {"op":"style_text","text_no":N,"size":..(선택),"color":"#RRGGBB"(선택),"x":..(선택),"y":..(선택)}\n'
    "  (기존 자막의 크기·색·위치만 변경, 내용은 그대로. 색상 단어(빨강/파랑/노랑/흰색/검정 등)는 "
    "네가 알맞은 #RRGGBB로 변환해서 써라.)\n"
    ...
    '- {"op":"clip_animation","cut_no":N,"kind":"in|out","name":"애니메이션 느낌(예: 포커스/줌/흔들림)",'
    '"duration_s":초(선택,기본1)}  (그 컷의 입장(in)/퇴장(out) 애니메이션)\n'
