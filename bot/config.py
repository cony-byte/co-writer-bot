...
# ── 구독 에이전트 라우터(bot/agent_router.py) — 2026-07-21 신규 ──
# OpenRouter 단발 분류기(tool_router) 대신 Claude Agent SDK(구독/팀 로그인, 키 불필요)로
# 스스로 여러 번 도구를 호출하는 멀티턴 에이전트 라우터. "전면 에이전트화" 선택에 따라 기본값.
# 롤백: .env에 COWRITER_ROUTER_BACKEND=openrouter (한 줄) → 기존 tool_router 경로 그대로.
ROUTER_BACKEND = os.environ.get("COWRITER_ROUTER_BACKEND", "agent")
# 타임아웃: 사용자 요청대로 "아주 널널". 24/7 KeepAlive 봇에서 CLI 서브프로세스가 응답 없이
# 매달릴 때 스레드가 영원히 새는 걸 막기 위해 무한(None)은 쓰지 않고, 30분으로 크게 바운드.
AGENT_ROUTER_TIMEOUT = int(os.environ.get("COWRITER_AGENT_ROUTER_TIMEOUT", "1800"))
# 멀티스텝 계획이 중간에 잘리지 않게 max_turns도 넉넉히.
AGENT_ROUTER_MAX_TURNS = int(os.environ.get("COWRITER_AGENT_ROUTER_MAX_TURNS", "40"))
