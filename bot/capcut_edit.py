...
- 오디오: 배경음악 교체(replace_audio, 없으면 새로 추가) / 볼륨(volume) / 음소거(mute) / 페이드(audio_fade)
- 자막: 자막 추가(add_text, 컷에 맞춰) / 자막 수정(edit_text) / 자막 스타일(style_text, 크기·색·위치)
- 필터: 컷에 CapCut 필터 적용(filter)
- 애니메이션: 컷·자막 입장/퇴장 애니메이션(clip_animation/text_animation)
- 클립: 컷의 영상 소재 교체(replace_clip) / 새 컷 삽입(insert) / 분할(split) / 복제(duplicate)
- 화면: 배경 채우기(background_fill, 블러/색) / 줌·팬 키프레임(zoom_pan, 켄번즈 효과)

미지원(범위 밖): 스티커/이미지 오버레이, 마스크, 정교한 색보정 컴포지팅."""
...
def _new_canvas_material(style: str, blur: float, color: str | None) -> dict:
    """배경 채우기(블러/색) material(실측 확인, 2026-07-23) — 세로/가로 비율이 안 맞는 클립
    뒤에 블러 처리된 배경이나 단색 배경을 깐다."""
    return {"id": uuid.uuid4().hex, "type": "canvas_blur" if style != "color" else "canvas_color",
            "blur": blur if style != "color" else 0.0, "color": color or "#00000000",
            "source_platform": 0}


def _new_keyframe_track(property_type: str, points: list[tuple[int, float]]) -> dict:
    """줌/팬(켄번즈) 등 키프레임 트랙(실측 확인, 2026-07-23) — points=[(time_offset_us, value), ...].
    property_type은 pyCapCut KeyframeProperty의 실제 값 문자열(예: 'KFTypeScaleX')."""
    kfs = [{"curveType": "Line", "graphID": "", "left_control": {"x": 0.0, "y": 0.0},
            "right_control": {"x": 0.0, "y": 0.0}, "id": uuid.uuid4().hex,
            "time_offset": t, "values": [v]} for t, v in points]
    return {"id": uuid.uuid4().hex, "keyframe_list": kfs, "material_id": "", "property_type": property_type}
    ...
    for op in ops:
        ...
        try:
            ...
            # ── 클립 교체/삽입 ──────────────────────────────────
                ...
                cut_no = int(op.get("cut_no", 1))
                if not (1 <= cut_no <= len(vt["segments"])):
                    logs.append(f"컷#{cut_no} 없음 — 분할 건너뜀")
                    continue
                seg = vt["segments"][cut_no - 1]
                dur = seg["target_timerange"]["duration"]
                at = int(float(op.get("at_s", 0)) * _US)
                if not (0 < at < dur):
                    logs.append(f"컷#{cut_no} 분할 실패 — 분할 지점이 컷 길이({dur/_US:.1f}s) 범위 밖")
                    continue
                src = seg.get("source_timerange") or {"start": 0, "duration": dur}
                seg2 = copy.deepcopy(seg)
                seg2["id"] = uuid.uuid4().hex
                seg["target_timerange"] = {"start": seg["target_timerange"]["start"], "duration": at}
                seg["source_timerange"] = {"start": src["start"], "duration": at}
                seg2["target_timerange"] = {"start": 0, "duration": dur - at}
                seg2["source_timerange"] = {"start": src["start"] + at, "duration": dur - at}
                seg2["extra_material_refs"] = []  # 트랜지션/필터/애니메이션은 앞쪽 조각에만 남김(간단화)
                vt["segments"].insert(cut_no, seg2)
                logs.append(f"컷#{cut_no}을 {op.get('at_s')}s 지점에서 분할")
            elif kind == "duplicate":
                cut_no = int(op.get("cut_no", 1))
                if not (1 <= cut_no <= len(vt["segments"])):
                    logs.append(f"컷#{cut_no} 없음 — 복제 건너뜀")
                    continue
                dup = copy.deepcopy(vt["segments"][cut_no - 1])
                dup["id"] = uuid.uuid4().hex
                vt["segments"].insert(cut_no, dup)
                logs.append(f"컷#{cut_no} 복제")
            elif kind == "background_fill":
                cut_no = int(op.get("cut_no", 1))
                if not (1 <= cut_no <= len(vt["segments"])):
                    logs.append(f"컷#{cut_no} 없음 — 배경 채우기 건너뜀")
                    continue
                style = "color" if op.get("style") == "color" else "blur"
                cmat = _new_canvas_material(style, float(op.get("blur", 0.0625)), op.get("color"))
                content.setdefault("materials", {}).setdefault("canvases", []).append(cmat)
                vt["segments"][cut_no - 1].setdefault("extra_material_refs", []).append(cmat["id"])
                logs.append(f"컷#{cut_no} 배경 채우기({style})")
            elif kind == "zoom_pan":
                cut_no = int(op.get("cut_no", 1))
                if not (1 <= cut_no <= len(vt["segments"])):
                    logs.append(f"컷#{cut_no} 없음 — 줌/팬 건너뜀")
                    continue
                seg = vt["segments"][cut_no - 1]
                dur = seg["target_timerange"]["duration"]
                cks = seg.setdefault("common_keyframes", [])
                if "from_scale" in op or "to_scale" in op:
                    fs = float(op.get("from_scale", 1.0)); ts = float(op.get("to_scale", fs))
                    cks.append(_new_keyframe_track("KFTypeScaleX", [(0, fs), (dur, ts)]))
                    cks.append(_new_keyframe_track("KFTypeScaleY", [(0, fs), (dur, ts)]))
                if "from_x" in op or "to_x" in op:
                    fx = float(op.get("from_x", 0.0)); tx = float(op.get("to_x", fx))
                    cks.append(_new_keyframe_track("KFTypePositionX", [(0, fx), (dur, tx)]))
                if "from_y" in op or "to_y" in op:
                    fy = float(op.get("from_y", 0.0)); ty = float(op.get("to_y", fy))
                    cks.append(_new_keyframe_track("KFTypePositionY", [(0, fy), (dur, ty)]))
                logs.append(f"컷#{cut_no} 줌/팬 키프레임 추가")
            else:
