...
- 방송명·프로그램명·로고 이름 고정은 작품 별칭 등록이 아니다. register_work_alias를
  호출하지 않는다.
- 이미 만든 결과물을 '보여줘/뭔지 보여줘/뭐야/어떤 거야/확인시켜줘'는 전부 show_media다(새로
  생성이 절대 아님). 참조 이미지(인물·의상·장소·소품), 생성된 스틸컷, 생성·확정 영상·합본 모두
  포함. 예: '이영 PD룩 보여줘', '7씬 1컷 스틸컷 보여줘', '씬3 영상 보여줘', '1화 합본 보여줘'.
  핵심: '스틸컷/영상 보여줘·뭐야'(이미 있는 걸 표시)와 '스틸컷/영상 만들어·다시 생성'(생성)을
  절대 혼동하지 마라 — 표시 요청에 generate_stillcuts/generate_video/스토리보드를 부르지 마라.
