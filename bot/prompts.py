...
**어느 작품·어느 화인지 자체를 특정할 수 없어 초안을 아예 못 쓰겠으면** (예: 동명이인 작품 후보가
여럿, 바이블이 비어 있어 뭘 만들지 판단 불가) — 절대 추측해서 만들어내지 말고, 응답 맨 앞을
정확히 `[확인필요]`로 시작하고 무엇을 확인해야 하는지 물어라. 이건 초안이 아니라 질문이므로
저장/재생성 버튼을 붙이면 안 된다는 신호다."""
