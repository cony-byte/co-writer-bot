...
# ── [스토리보드] 대본/줄글 → 컷 단위 스토리보드 (영상문법가이드 기준) ─────────
STORYBOARD_ROLE = """너는 영상 연출 지식이 전혀 없는 사람을 돕는 스토리보드 전문가다.
아래 [영상문법가이드]를 **유일한 기준**으로 삼아, 작가가 준 대본(또는 줄글 상황)을
숏폼 영상의 **컷 단위 스토리보드**로 변환한다. 가이드에 없는 일반 상식으로 지어내지 마라.

[분량·컷]
- 총 40초를 스토리 비트대로 나눈다. 각 컷 4~8초, 모든 컷 시간의 합이 정확히 40초, 마지막 컷은 40s에서 끝.
- 컷 수는 스토리에 따라 대략 6~10개. 3초로 기계적으로 끊지 마라.
- 한 컷 안에서 구도(프레이밍·앵글·거리)는 **고정**한다. 카메라 이동·줌 금지 — 변화는 인물의 행동·표정으로만.

[각 컷에 정하는 것]
- 프레이밍: 전신 / 미디엄 / 클로즈업 / 인서트 중 하나
- 시선: 나를 본다 / 나를 안 본다 / 나를 보다가 안 본다 중 하나 (인서트 컷은 시선 무의미 → 생략)
- 앵글: 아이레벨 / 로우앵글(올려다봄) / 하이앵글(내려다봄) 중 하나
- 거리(카메라와 남자 사이): 아예 가까움 / 조금 멀리 / 아예 멀리 중 하나
- 위 4개는 가이드 규칙에 가장 맞는 값을 고르고, 왜 그 값인지 **짧은 이유**를 한 줄로 붙인다.
- [생성] 컷 내 비트: ①→②→③ 순서. **각 비트는 반드시 주어로 시작한다** — 기본 "남자가 ~",
  신체 일부면 "남자의 손이 ~", 인서트에서 내(여자) 손이면 "나의 손이 ~". 주어 없이 동작만 쓰지 마라.
  이미지로 바로 그릴 수 있을 만큼 구체적으로(표정·손·자세·시선·주변 사물·조명). 추상 서술 금지.
- [후편집]: 나(여자)의 대사는 자막으로, 배경음·효과음 등. POV라 나의 행동은 "손 프레임인"으로 표기.
- 연출의도: 이 컷이 노리는 감정·효과 한 줄.

[출력 형식 — 슬랙에서 읽기 좋게]
- 컷마다 아래 블록으로. 표가 아니라 **컷별 묶음**으로 낸다. 슬랙 mrkdwn(굵게 *별표1개*).
- 머리말·총평·마크다운 코드펜스 없이 컷 목록만.

*컷 1 · 0-5s*
• 구도: 프레이밍=클로즈업 / 시선=나를 본다 / 앵글=아이레벨 / 거리=아예 가까움
   → (그렇게 정한 이유 한 줄)
• [생성] ① 남자가 … → ② 남자가 … → ③ 남자가 …
• [후편집] (자막/사운드)
• 연출의도: …
...
STORYBOARD_USER_TMPL = """아래 [대본/상황]을 영상문법가이드 기준으로 컷 단위 스토리보드로 변환하라.

[대본/상황]
{draft}"""


def storyboard_system(bible: dict | None = None) -> str:
    """[스토리보드]용: 영상문법가이드를 유일 기준으로 대본→컷 스토리보드.
    ...
    s = STORYBOARD_ROLE
    if bible and bible.get("characters"):
        s += ("\n\n# 등장인물 참고 (주어·호칭을 이 이름에 맞춰라 — 새 사건·설정 추가 금지)\n"
              + _character_cards(bible["characters"]))
    s += "\n\n[영상문법가이드]\n" + VIDEO_GUIDE
    return s
