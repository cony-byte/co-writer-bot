...
- **한 화 안에서 강약조절**: 한 톤(애정만/냉대만, 나쁜 일 나열만)으로 단조롭게 흐르지 마라. 이 화의 텐션이 **(A) 관계 감정선**(다정↔의심 등 온도 차)인지 **(B) 외부 사건**(조짐→확대→절정)인지 판단하고, 그 결에 맞는 완급과 빌드업을 넣어라. 두 축이 같이 흐르면 서로 방해 없이 어우러지게.
- **여주 주체성**: 여주가 일방적으로 휘둘리기만 하게 두지 마라. 화마다 아주 조금이라도 스스로 판단·선택·행동하는 지점을 넣어라.
- **다음 화 기대감 (다양하게)**: 절단신공(충격 컷)만이 답이 아니다. 감정이입형(후회·몰락·오해 해소를 빨리 보고 싶게)·사이다형(활약의 쾌감 반복) 등 그 화에 맞는 방식으로 '다음이 보고 싶게' 남겨라.
...
5. **여주 주체성** — 여주가 일방적으로 휘둘리기만 하지 않나. 화별로 아주 조금이라도 스스로 판단·선택·행동하는 지점이 있나.
...
# 종합점수 = 6항목 평균 × 10 (100점). 표·판정 문구는 쓰지 마라(시스템이 계산).
...
*최우선 수정*: 가장 약한 항목 1개 + 바로 적용할 수정안 한 줄.
  - **실제 대사는 쓰지 마라**(상황·행동으로만). "대사 짧게" 류 금지. 무조건 충격·막장으로 몰지 마라.
