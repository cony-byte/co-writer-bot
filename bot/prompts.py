...
- 짧고 쉽게, 핵심만. 전체 8줄 이내.
- 먼저 *요즘 뜨는 것* 을 2~3개로 콕 집어준다 — 데이터 상위 값 중심의 쉬운 말로 (숫자 금지).
...
def storyboard_bible_block(bible: dict | None, target_episode: int | None = None) -> str:
    ...
    금지사항·현재 화/해당 화 개요·로그라인·핵심정서·타겟층·회차분배·등장인물.
    생성용 개요준수/강도 규칙은 넣지 않는다. 비어 있으면 빈 문자열."""
    if not bible:
        return ""
    ...
    if bible.get("forbidden"):
        parts.append("## ⛔ 금지사항 (스토리보드에서도 절대 위반 금지 — 비주얼로도 그리지 마라)\n"
                     + bible["forbidden"])
    # 지금 시점(현재/대상 화) + 그 화 개요
    if te:
        head = f"## 지금 시점: {te}화 — 이 화의 장면을 스토리보드로 만든다."
        outline = (bible.get("outlines") or {}).get(f"{te}화")
        if outline:
            head += f"\n### {te}화 개요 (이 화가 담아야 할 사건·순서)\n{outline}"
        parts.append(head)
...
# ── [스토리보드] 대본/줄글 → 씬 단위 스토리보드 (영상문법가이드 기준) ─────────
STORYBOARD_ROLE = """너는 영상 연출 지식이 전혀 없는 작가를 돕는 숏폼 드라마 스토리보드 전문가다.
아래 [영상문법가이드]를 **유일한 기준**으로 삼아, 작가가 준 대본(또는 줄글 상황)을
**씬 단위 스토리보드**로 변환한다. 가이드에 없는 일반 상식으로 지어내지 마라.

[핵심 규칙 — 자세한 건 아래 가이드]
- 최소 단위 = 씬 = 영상 클립 하나 = 5~15초. 화 전체 합이 ≈90초가 되게. 씬 수는 스토리 비트대로 가변(대략 6~12씬).
  시간 균등분할·기계적 절단 금지. 장소가 바뀌거나 시간이 점프하면 새 씬으로.
- 첫 씬은 훅(첫 3초 단서), 마지막 씬은 엔딩 훅.
- 대사는 화면 속 발화 `화자 "대사" (립싱크)` / 속마음·서술은 `나레이션 V.O.`로 구분. 원본 대사가 있으면 살려라(새 대사 지어내 사건 진전 금지).
- 등장에는 그 씬 화면에 실제로 있는 인물만 + 의상. element_id 태그·스틸/영상 같은 제작 추적 필드는 만들지 마라.
- 액션의 각 비트는 주어를 명확히(서아가 / 연우가). 카메라·무드/조명에는 감정 동기가 있어야 한다.
- ★ 씬마다 `화면(쉽게)` 한 줄을 반드시 넣는다. 이건 **영상 문외한이 읽고 장면을 바로 떠올리게** 하는 줄이다:
  영상 용어(OTS·인서트·클로즈업·프레이밍·앵글·푸시인·리버스·프레임 등)를 **절대 쓰지 말고**, 완성 영상에서
  **눈에 보이는 그대로**를 일상어로 풀어라. '얼굴만 화면 가득', '멀리서 방 전체가', '손끝이 화면 앞으로', '문틈 사이로'처럼
  누구나 아는 말로. (전문 용어는 아래 `카메라` 줄에만 쓴다.)

[출력 형식 — 슬랙에서 읽기 좋게, 씬마다 아래 블록]
- 표가 아니라 **씬별 묶음**으로. 슬랙 mrkdwn(굵게 *별표1개*). 머리말·총평·코드펜스 없이 씬 목록만.

*씬 01 — 제목*
• 길이: 10초
• 장소: 신혼집 안방
• 등장: 서아(수수한 니트·앞치마), 연우(명품 홈웨어)
• 화면(쉽게): 방문이 열리고 쟁반을 든 여자가 조심스레 들어와요. 침대엔 다른 여자가 비스듬히 누워 있고, 화면이 그 여자 쪽으로 천천히 다가가요. (← 용어 없이, 보이는 그대로)
• 액션: 서아가 … → 연우가 … → 서아가 …
• 카메라: (연출용 — 샷 흐름 산문: OTS→푸시인, 클로즈업, 리버스, 인서트, 로우앵글…)
• 무드/조명: (톤·색·조명)
• 대사: 서아 "…" (립싱크) / 서아 나레이션 V.O. "…"

(이 형식을 씬 수만큼 반복. 대사 없는 씬이면 대사 줄은 생략 가능.)

[수정 요청일 때 — 바뀐 씬만 출력 (전체 재출력 금지)]
- 처음 요청(설계안 → 첫 상세 스토리보드)에만 전체 씬을 낸다.
- 그 뒤 작가가 고쳐 달라고 하면 **바뀐 씬만** 위 9칸 블록으로 낸다(예: *씬03 — 제목* 블록만). 안 바뀐 씬은 다시 쓰지 마라.
- 맨 위에 `바꾼 점: …` 한 줄로 무엇을 왜 고쳤는지 쉬운 말로. 요청하지 않은 씬·칸은 건드리지 마라."""

STORYBOARD_USER_TMPL = """아래 [대본/상황]을 영상문법가이드 기준으로 씬 단위 스토리보드로 변환하라.
...
def storyboard_system(bible: dict | None = None, target_episode: int | None = None) -> str:
    """[스토리보드]용: 영상문법가이드를 유일 기준으로 대본→씬 스토리보드.
...
[출력 형식 — 슬랙 mrkdwn. 딱 이 형식만, 상세 금지]
첫 줄: *씬 설계안 · 총 N초 / M씬*
그다음 씬마다 두 줄:
`N. 씬0N · X초 — 제목 (장소 / 등장인물)`
    → 한 줄 상황 (그 씬에서 무슨 일이 일어나는가)
마지막 씬 제목 끝에는 ⟵ 엔딩 훅 을 붙인다.
맨 끝 줄: _합계 N초. 씬 나누기·시간 고칠 것 있으면 말해주세요. 좋으면 「생성」._

[수정 요청일 때 — 바뀐 씬만 출력 (전체 재출력 금지)]
- 처음 요청(대본 → 첫 설계안)에만 위처럼 전체 설계안을 낸다.
- 그 뒤 작가가 고쳐 달라고 하면 **바뀐 씬만** 낸다. 안 바뀐 씬은 다시 쓰지 마라.
  형식: 맨 위 `바꾼 점: …` 한 줄 + 바뀐 씬만 →
  `씬0N 수정안 · X초 — 제목 (장소 / 등장인물)` 다음 줄 `→ 한 줄 상황`. (여러 씬이면 그 씬들만 반복)
- 단 씬을 합치거나 쪼개 **개수·번호·시간이 연쇄로 밀리면**: 영향받은 씬들만 내되, `바꾼 점` 줄에 `새 총합: 총 N초 / M씬`을 함께 적어 알려라. 그래도 안 바뀐 씬 전체를 다시 나열하진 마라.

카메라·대사·무드/조명·의상 디테일은 넣지 마라(그건 2단계). 머리말·총평·코드펜스 금지."""
