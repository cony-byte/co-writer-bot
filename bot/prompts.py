...
# 가중치 (합계 100): 훅 25 · 엔딩절단점 25 · 감정터짐 20 · 전개속도 15 · 대사맛 10 · 장면이해 5
...
FUN_USER_TMPL = """아래 [대본]을 시청자 입장에서 평가하라.

━━ 0. 이탈 시점 추정 (가장 먼저 답하라)
영상으로 본다면 몇 초/어느 장면에서 스크롤을 넘길 가능성이 가장 높은가?
"끝까지 봄" 또는 "이탈 예상: ○○ 장면 진입부 (이유)"로 답하라.

━━ 1. 항목 평가 — 각 항목: `[점수 n/10]` + 한 줄 판정 + 대본 속 근거 인용
① 훅(첫 3초) [가중치25] — 첫 대사·장면이 손을 멈추게 하나? (9~10: 첫 컷이 사건 한복판(뺨·불륜 목격·정체 폭로) / 3~4: 인물소개·상황설명·일상대화로 시작)
② 전개 속도 [가중치15] — 늘어지거나 설명이 긴 구간이 있나, 어디인지. (9~10: 모든 장면이 사건 전진 / 3~4: 정보 반복·설명대사 3줄 이상·긴 감정정리)
③ 감정 터짐 [가중치20] — '와' 하는 순간이 몇 번, 어느 장면. (9~10: 굴욕 정점·반전·감정폭발 2회 이상 / 3~4: 평탄)
④ 대사 맛 [가중치10] — 가장 약한 대사 1개와 가장 좋은 대사 1개를 반드시 인용. (9~10: 성격 밴 짧은 말 / 3~4: 설명조·번역체)
⑤ 장면 이해 [가중치5] — 앞 화 안 본 시청자도 3초 안에 상황이 잡히나, 헷갈리는 지점. (9~10: 행동만으로 파악 / 3~4: 관계 알아야 이해·전환 불친절)
⑥ 엔딩 절단점 [가중치25] — 다음 화를 못 참게 하나? (9~10: 반전 대사 한 방·폭로 직전 컷·결심 선언 직후 / 3~4: 감정정리·여운·사건 완결)

━━ 2. 종합점수 = (①×25 + ②×15 + ③×20 + ④×10 + ⑤×5 + ⑥×25) ÷ 10
점수와 계산을 표로 보여라.

━━ 3. 강제 지목 — 가장 약한(점수 낮은) 항목 1개를 지목하고, 바로 적용 가능한 구체 수정안 1개를 제시하라. (방향 제시 금지)

━━ 4. 종합 판정 (한 줄) — [그대로 가도 됨 / 이것만 고치면 됨: ○○ / 약점 2개 이상 수술 / 구조부터 다시]
...
FEEDBACK_LOGIC = """*개연성 오류* (지적만 — 고치는 건 작가 몫이니 수정안은 달지 마라)
- 인물 설정·관계·말투, 앞 화 흐름, 회차분배 핵심사건, 금지사항과 **어긋나는 지점**만 짚어라.
- 캐릭터 붕괴(갑자기 성격 바뀜), 시점 오류(아직 모를 정보를 앎), 급전개(아직 이른 사건)를 특히 본다.
- 각 오류는 두 줄로: `- [문제] 무엇이 어긋나는지 한 줄` / `  [근거] 바이블 어디와 충돌하는지 한 줄`
- 수정안·대안·"이렇게 하라"는 제안은 붙이지 마라.
- 어긋나는 게 없으면 "개연성 문제 없음" 한 줄로."""
...
def _outlines_block(outlines: dict, before: int | None = None) -> str:
    ...
    return ("## 지난 화 개요 (지금까지 쓴 것 전부 — 흐름을 이어가되 같은 사건 반복 금지)\n" + body)
