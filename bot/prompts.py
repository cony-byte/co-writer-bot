...
아래 [작품 바이블]은 이 작품의 확정 설정이다. **{target} 시점** 기준으로 다음을 스스로 점검해 어긋나지 않게 한다.

1. **시점에 안 맞는 캐릭터 차단** — 등장인물·회차분배에서 각 인물의 등장/활동 구간·포지션을 보고, 지금 시점에 없거나 회상으로만 나올 인물을 현재 장면에 넣지 마라.
2. **캐릭터 붕괴 차단** — 인물의 포지션·설정·핵심대사(말투)를 지켜라. 특정 화까지의 태도(예: 냉대)가 바이블에 있으면 그 전환 시점을 지킨다.
3. **급전개/무전개 차단** — 줄거리·회차분배상 "이 시점엔 아직 일어나지 않을 사건"을 미리 터뜨리지 마라.
4. **개요 준수(대본일 때)** — 이 화의 [개요]가 있으면 그 사건·순서·엔딩을 **반드시** 따른다. 개요에 없는 전개를 지어내지 마라.
5. **참조 규칙** — 회차분배는 줄거리를, 대본은 그 화 개요를 근거로 한다.
6. **톤·문법** — 아래 기존 화 대본의 나레이션·대사 문법(화자 표기·문체·호흡)을 그대로 따라라.
...
def build_bible_block(bible: dict, target_episode: int | None = None) -> str:
    ...
    target = f"{target_episode}화" if target_episode else "현재"
    parts = [FAILSAFE.replace("{target}", str(target)), f"\n# [작품 바이블] {bible.get('title')}"]
    if bible.get("stale"):
